...
?>
<form name="frmSearch"  method="post" action="<?php  echo site_url("admin/model") ?>" >
    <article class="module width_full">
            <header>
                    <h3> Search </h3>
            </header>
            <div class="module_content">
                Make Name :  <input type="text" style="width : 150px;" name="make_name" value="<?php echo $make_name; ?>" />  &nbsp;
...
<article class="module width_full">
    <header>
        <h3 class="tabs_involved"> model List </h3>
    ...
    <div class="tab_container">
        <table class="tablesorter" cellspacing="0">
            ...
            foreach($model_list as $row) {
                 $make=$this->manage_m->get_specific_data($row['make_id'], "makes");
            ?>
                <tr>				
                    <td><?php echo $i+$start_no; //$row['id'];?></td>
                    <td><?php echo $row['id'];?></td>                    
                    <td><?php echo $row['name'];?></td>
                    <td><?php echo $make!=null ? $make['name'] : "<div style='color:red'> No Available Make </div>";?></td>
