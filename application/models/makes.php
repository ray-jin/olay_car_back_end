...
class Makes extends CI_Model 
{
	...
	function &_create_pagenation($per_page,$total,$base_url) {
     ...
     //&get_object_list( $start=0, $count=1000, $search_option='') 
        ...
        function list_makes($start,$count,$name)
        {            
            $strSql = "SELECT count(*) as cnt FROM makes m WHERE m.name LIKE '%$name%' ORDER BY m.name ";
                        
            $query = $this->db->query($strSql);		
            $row = $query->row_array();
            $return_arr['total'] = $row['cnt'];
            
            $strSql = "SELECT m.* FROM makes m WHERE m.name LIKE '%$name%' ORDER BY m.name limit $start, $count ";
